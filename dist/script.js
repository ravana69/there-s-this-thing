...
async function Draw(){
  ...
  if(!t){
    ...
    Array(cl*rw*br).fill().map((v,i)=>{
      ...
      d = Math.hypot(Y,Z)
      if(d<sp*rw/2) grid = [...grid, [X,Y,Z,1]]
    })
    ...
    spawnSparks = (X,Y,Z) => {
      for(m=4;m--;){
        vx = S(p=Math.PI*2*Rn()) * S(q=Rn()<.5?Rn()**.5*Math.PI/2:Math.PI-Rn()**.5*Math.PI/2) * iSparksv
        vy = C(q) * iSparksv
        vz = C(p) * S(q) * iSparksv
  ...
  oX=0, oY=0, oZ=Math.min(20,Math.max(8,(.3+S(t/2))*80))
  Rl=t/4, Pt=-S(t/2)*2, Yw = S(t/8)
  ...
  P.map(v=>{
    ...
    if(Z>0){
      l = Q()
      s = Math.min(3e3,2500/Z*v[6])
      x.fillStyle = '#00ff0015'
      x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
      x.fillStyle = '#00ffff40'
      s/= 2
      x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
